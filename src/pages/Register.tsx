  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (referralCode.trim()) {
        ...
        else toast.warning('Referral code not found — registering without referral');
      }
      ...
      console.error(err);
      toast.error(err.message || 'Failed to create account. Check Supabase connection.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="app-container min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 px-5 py-6 space-y-4">
        ...
        <div>
          ...
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your full name" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 outline-none bg-white text-sm" />
        </div>

        <div>
          <label className="text-sm font-semibold text-gray-600 mb-1.5 block">Phone Number</label>
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="e.g. 0712345678" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 outline-none bg-white text-sm" />
        </div>

        <div>
          <label className="text-sm font-semibold text-gray-600 mb-1.5 block">Password</label>
          <div className="relative">
            <input type={showPassword? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 6 characters" className="w-full border border-gray-200 rounded-xl px-4 py-3 pr-12 text-gray-800 outline-none bg-white text-sm" />
        ...
        <div>
          ...
          <div className="relative">
            <input type={showConfirm? 'text' : 'password'} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Re-enter password" className="w-full border border-gray-200 rounded-xl px-4 py-3 pr-12 text-gray-800 outline-none bg-white text-sm" />
        ...
        <div>
          ...
          <input type="text" value={referralCode} onChange={(e) => setReferralCode(e.target.value.toUpperCase())} placeholder="Enter referral code" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-gray-800 outline-none bg-white text-sm uppercase tracking-wider" />
        </div>

        <button onClick={handleRegister} disabled={submitting} className="w-full py-4 rounded-xl text-white font-bold text-base transition-all active:scale-95 disabled:opacity-60 mt-2" style={{ background: 'linear-gradient(135deg, #1d4ed8, #3b82f6)' }}>
