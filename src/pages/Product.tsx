...
const Product = () => {
  ...
  const loadData = async () => {
    ...
    const [prods, recharges] = await Promise.all([
      getUserProducts(freshUser.id),
      getUserRecharges(freshUser.id),
    ]);
    setMyProducts(prods);
  ...
  const handleBuy = async (packageId: string, price: number, packageName: string) => {
    ...
    if (user.balance < price) {
      toast.error(`Insufficient balance. You need ${fmt(price)} but have ${fmt(user.balance)}.`);
      return;
    }
      ...
      <div className="p-4 space-y-4">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <div className="text-yellow-700 text-xs mt-0.5">
                  You have a pending recharge of <strong>{fmt(totalPending)}</strong> awaiting approval — balance will update once approved.
                </div>
              </div>
            </div>
          </div>
        ...
        {user.frozen && (
          <div className="bg-red-50 border border-red-300 rounded-xl p-4 text-red-700 text-sm font-medium">
            ⚠️ Your account is frozen. Package purchases are disabled.
          </div>
        )}
                      ...
                      <div className="p-4">
                        ...
                        {owned ? (
                          ...
                          <button
                            ...
                            disabled={!!buyingId || user.frozen || loading}
                            className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-2.5 rounded-xl transition text-sm"
                          >
                            {buyingId === pkg.id ? 'Processing...' : `Buy — ${fmt(pkg.price)}`}
                          </button>
                        )}
                      </div>
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            ...
            <div className="bg-gray-50 rounded-xl p-3 mb-4 text-sm text-gray-600">
              <strong>{fmt(proofModal.price)}</strong> will be deducted from your balance (<strong>{fmt(user.balance)}</strong>).
              The package will be activated after admin review.
            </div>
            <textarea
              value={proofText}
              onChange={e => setProofText(e.target.value)}
              placeholder="Payment reference or proof (optional)"
            ...
            <div className="flex gap-2 mt-4">
              <button
                onClick={() => { setProofModal(null); setProofText(''); }}
                className="flex-1 py-2.5 border rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 transition"
              >
                Cancel
              </button>
              <button
                onClick={confirmBuy}
                disabled={!!buyingId}
                className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white rounded-xl text-sm font-semibold transition"
              >
                {buyingId ? 'Processing...' : 'Confirm Buy'}
              </button>
            </div>
          </div>
        </div>
