// GITHUB ONLY - WORKING - DO NOT ADD SUPABASE IMPORT
...
export function getCurrentUser(){
  for(const k of CUR_KEYS){
    try{
      ...
      if(v){const u=JSON.parse(v); if(u&&u.id) return u;}
    }catch{}
  }
  return null;
}
...
export async function getUsers(){return loadUsers();}
export async function getUserById(id:string){return loadUsers().find((x:any)=>x.id===id)||null;}
export async function getUserByPhone(phone:string){return loadUsers().find((x:any)=>x.phone===phone)||null;}
export async function getUserByReferralCode(code:string){return loadUsers().find((x:any)=>x.referralCode===code.toUpperCase())||null;}
export async function createUser(user:any){const a=loadUsers();a.push(user);saveUsers(a);}
export async function updateUser(id:string,up:any){const a=loadUsers();const i=a.findIndex((x:any)=>x.id===id);if(i>=0){a[i]={...a[i],...up};saveUsers(a);const cur=getCurrentUser();if(cur?.id===id)setCurrentUser(a[i]);}}
export async function deleteUserById(id:string){saveUsers(loadUsers().filter((x:any)=>x.id!==id));}
