...
export const processDailyIncome = async (): Promise<void> => {
  ...
  for (const product of products) {
    ...
    const lastDate = product.lastIncomeDate ? new Date(product.lastIncomeDate) : null;
    if (lastDate) {
      const diffHours = (now.getTime() - lastDate.getTime()) / (1000 * 60 * 60);
      if (diffHours < 23) continue;
    }
    ...
    if (user) {
      ...
      await addNotification({
        ...
        message: `UGX ${product.dailyIncome.toLocaleString()} has been credited to your account from ${product.packageName}.`,
        isRead: false,
      });
    }
  }
