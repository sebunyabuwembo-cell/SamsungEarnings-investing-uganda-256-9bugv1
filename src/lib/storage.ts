/**
 * storage.ts — Fixed for Eagle Investment (GitHub Pages Offline Mode)
 */
...
export function getCurrentUser(): User | null {
  try {
    ...
    if (typeof parsed === 'string') {
      const users = JSON.parse(localStorage.getItem('engle_users') || '[]');
      return users.find((u: any) => u.id === parsed) || { id: parsed } as any;
    }
    if (parsed && parsed.id &&!parsed.phone) {
      const users = JSON.parse(localStorage.getItem('engle_users') || '[]');
      const found = users.find((u: any) => u.id === parsed.id);
      return found || parsed;
    }
...
export function setCurrentUser(user: User | string | null): void {
  ...
  if (typeof user === 'string') {
    const placeholder = { id: user };
    localStorage.setItem(SESSION_KEY, JSON.stringify(placeholder));
    sessionStorage.setItem(SESSION_KEY, JSON.stringify(placeholder));
    return;
  }
...
export async function refreshCurrentUser(): Promise<User | null> {
  ...
  const fresh = await getUserById(cached.id) || await getUserByPhone((cached as any).phone || '');
...
export async function runDailyIncomeWithStats(): Promise<{ processed: number; totalPaid: number; errors: number }> {
  ...
  for (const product of activeProducts) {
    try {
      ...
      stats.processed++; stats.totalPaid += income;
