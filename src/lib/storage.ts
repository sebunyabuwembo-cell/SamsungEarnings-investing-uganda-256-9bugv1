...
export async function createUser(userData: {
  ...
  const { data, error } = await supabase.from('samsung_users').insert([userData]).select().single();
...
export async function updateUser(id: string, updates: Record<string, unknown>): Promise<void> {
  ...
  try {
    ...
    if (raw) {
      ...
      if (cached.id === id) {
        const merged = {...cached };
        if ((updates as any).balance!== undefined) merged.balance = (updates as any).balance;
        if ((updates as any).total_earnings!== undefined) merged.totalEarnings = (updates as any).total_earnings;
...
export async function createProduct(productData: Record<string, unknown>): Promise<UserProduct | null> {
  const { data, error } = await supabase.from('samsung_products').insert([productData]).select().single();
...
export async function getRecharges(): Promise<Recharge[]> {
  const { data, error } = await supabase.from('samsung_recharges').select('*').order('created_at', { ascending: false });
...
export async function getUserRecharges(userId: string): Promise<Recharge[]> {
  const { data, error } = await supabase.from('samsung_recharges').select('*').eq('user_id', userId).order('created_at', { ascending: false });
  if (error) { console.error('getUserRecharges error:', error); return []; }
  return (data || []).map(mapRecharge);
}

export async function createRecharge(rechargeData: Record<string, unknown>): Promise<Recharge | null> {
  const { data, error } = await supabase.from('samsung_recharges').insert([rechargeData]).select().single();
...
export async function getWithdrawals(): Promise<Withdrawal[]> {
  const { data, error } = await supabase.from('samsung_withdrawals').select('*').order('created_at', { ascending: false });
...
export async function getUserWithdrawals(userId: string): Promise<Withdrawal[]> {
  const { data, error } = await supabase.from('samsung_withdrawals').select('*').eq('user_id', userId).order('created_at', { ascending: false });
  if (error) { console.error('getUserWithdrawals error:', error); return []; }
  return (data || []).map(mapWithdrawal);
}

export async function createWithdrawal(wData: Record<string, unknown>): Promise<Withdrawal | null> {
  const { data, error } = await supabase.from('samsung_withdrawals').insert([wData]).select().single();
...
export async function getUserWallets(userId: string): Promise<Wallet[]> {
  const { data, error } = await supabase.from('samsung_wallets').select('*').eq('user_id', userId).order('created_at', { ascending: false });
...
export async function createWallet(walletData: {
  ...
  const { data, error } = await supabase.from('samsung_wallets').insert([walletData]).select().single();
  if (error) { console.error('createWallet error:', error); return null; }
  return mapWallet(data);
}

// ✅ FIX FOR YOUR BUILD ERROR - Withdraw.tsx expects saveWallet
export async function saveWallet(walletData: any): Promise<Wallet | null> {
  // support both old and new call styles
  if (walletData?.user_id) {
    return createWallet(walletData);
  }
...
export async function deleteWallet(id: string): Promise<void> {
  const { error } = await supabase.from('samsung_wallets').delete().eq('id', id);
...
export async function getNotifications(): Promise<Notification[]> {
  const { data, error } = await supabase.from('samsung_notifications').select('*').order('created_at', { ascending: false });
  if (error) { console.error('getNotifications error:', error); return []; }
  return (data || []).map(mapNotification);
}

export async function getUserNotifications(userId: string): Promise<Notification[]> {
  const { data, error } = await supabase.from('samsung_notifications').select('*').eq('user_id', userId).order('created_at', { ascending: false });
...
export async function addNotification(
  ...
  if (typeof userIdOrObj === 'string') {
    payload = { user_id: userIdOrObj, type: type!, title: title!, message: message!, is_read: false };
  } else {
    payload = {
  ...
  const { error } = await supabase.from('samsung_notifications').insert([payload]);
...
export async function markAllNotificationsRead(userId: string): Promise<void> {
  const { error } = await supabase.from('samsung_notifications').update({ is_read: true }).eq('user_id', userId).eq('is_read', false);
...
export async function getRedeemCodes(): Promise<RedeemCode[]> {
  const { data, error } = await supabase.from('samsung_redeem_codes').select('*').order('created_at', { ascending: false });
...
export async function getRedeemCodeByCode(code: string): Promise<RedeemCode | null> {
  const { data, error } = await supabase.from('samsung_redeem_codes').select('*').eq('code', code).maybeSingle();
  if (error ||!data) return null;
  return mapRedeemCode(data);
}

export async function createRedeemCode(codeData: any): Promise<void> {
  const { error } = await supabase.from('samsung_redeem_codes').insert([{
    id: codeData.id,
    code: codeData.code,
    amount: codeData.amount,
    created_at: codeData.createdAt || codeData.created_at,
    expires_at: codeData.expiresAt || codeData.expires_at,
    used_by: codeData.usedBy || codeData.used_by || [],
    is_active: codeData.isActive?? codeData.is_active?? true,
  }]);
...
export async function processDailyIncome(): Promise<number> {
  ...
  const { data: activeProducts } = await supabase.from('samsung_products').select('*').eq('status', 'active');
  if (!activeProducts) return 0;
  let processed = 0;
  for (const product of activeProducts) {
    const lastDate = product.last_income_date? new Date(product.last_income_date as string).toISOString().split('T')[0] : null;
    if (lastDate === todayStr) continue;
    const expiryDate = product.expiry_date? new Date(product.expiry_date as string) : null;
    ...
    await supabase.from('samsung_products').update({
      total_income_earned: Number(product.total_income_earned) + dailyIncome,
      last_income_date: now.toISOString(),
    }).eq('id', product.id);
    const { data: userRow } = await supabase.from('samsung_users').select('*').eq('id', product.user_id).single();
    if (userRow) {
      ...
      await addNotification(product.user_id as string, 'income', 'Daily Income Received', `You earned UGX ${dailyIncome.toLocaleString()} from your ${product.package_name} package.`);
    }
    processed++;
  }
  return processed;
...
export async function runDailyIncomeWithStats(): Promise<{ processed: number; totalPaid: number }> {
  ...
  const todayStr = now.toISOString().split('T')[0];
  const { data: activeProducts } = await supabase.from('samsung_products').select('*').eq('status', 'active');
  if (!activeProducts) return { processed: 0, totalPaid: 0 };
  let processed = 0;
  let totalPaid = 0;
  for (const product of activeProducts) {
    const lastDate = product.last_income_date? new Date(product.last_income_date as string).toISOString().split('T')[0] : null;
